<?php
/**
 * Home Dashboard - Display Reports
 */
...
// Require admin or technician access - redirect regular users to their reports page
...
if ($userRole === 'administrador') {
    // Admin sees all reports using the view
    $sql = "SELECT r.*, 
    ...
    $sql = "SELECT r.id, r.folio, r.titulo, r.descripcion, 
            ...
            (SELECT TOP 1 comentario 
             ...
             ORDER BY fecha_comentario DESC) AS detalle_accion
            FROM Reportes r
            LEFT JOIN Edificios e ON r.id_edificio = e.id
            ...
            INNER JOIN Prioridades p ON r.id_prioridad = p.id
            INNER JOIN Usuarios rep ON r.id_reportante = rep.id
...
$statsStmt = $userRole === 'administrador' 
    ? sqlsrv_query($conn, $statsSQL)
    : sqlsrv_query($conn, $statsSQL, [$usuario_id]);

$stats = ['total' => 0, 'en_proceso' => 0, 'resueltos' => 0, 'retrasados' => 0, 'pendientes' => 0, 'hoy' => 0];
if ($statsStmt && $statsRow = sqlsrv_fetch_array($statsStmt, SQLSRV_FETCH_ASSOC)) {
    $stats = array_merge($stats, $statsRow);
}

// Get chart data - monthly reports
$chartSQL = "SELECT 
    ...
    . ($userRole !== 'administrador' ? " AND r.id_reportante = ?" : "") . "
    GROUP BY YEAR(r.fecha_reporte), MONTH(r.fecha_reporte)
    ORDER BY YEAR(r.fecha_reporte), MONTH(r.fecha_reporte)";

$chartStmt = $userRole === 'administrador'
    ? sqlsrv_query($conn, $chartSQL)
    : sqlsrv_query($conn, $chartSQL, [$usuario_id]);
...
$typeStmt = $userRole === 'administrador'
    ? sqlsrv_query($conn, $typeSQL)
    : sqlsrv_query($conn, $typeSQL, [$usuario_id]);
...
?>
  ...
  <div class="container">
    ...
    <h2 class="fw-bold mb-4 text-center">
      <?= $userRole === 'administrador' ? 'Panel de Administración' : 'Mis Reportes' ?>
    </h2>
    ...
    <div class="row g-4 mb-4">
      <div class="col-md-3">
        <div class="card p-4 text-center">
          <h5>Total <?= $userRole === 'administrador' ? 'Solicitudes' : 'Reportes' ?></h5>
      ...
      <div class="col-md-3">
        <div class="card p-4 text-center">
          <h5><?= $userRole === 'administrador' ? 'Resueltas' : 'Resueltos' ?></h5>
      ...
      <div class="col-md-3">
        <div class="card p-4 text-center">
          <h5><?= $userRole === 'administrador' ? 'Retrasados' : 'Pendientes' ?></h5>
          <p class="stat-value text-danger"><?= $userRole === 'administrador' ? $stats['retrasados'] : $stats['pendientes'] ?></p>
          <p class="text-muted"><?= $userRole === 'administrador' ? 'Necesitan atención' : 'Sin atender' ?></p>
        </div>
      </div>
    </div>
    ...
    <!-- Tabla de problemas registrados (dinámica) -->
    <div class="chart-container mt-4">
      <h4 class="section-title">Listado de reportes</h4>
      <div class="table-responsive">
        <table class="table table-bordered table-striped align-middle">
          ...
          <tbody>
            ...
            <?php if (!$hasReports): ?>
              <tr>
                <td colspan="<?= $userRole === 'administrador' ? '9' : '8' ?>" style="text-align: center; padding: 40px;">
                  <i class="fas fa-inbox" style="font-size: 48px; color: #ccc;"></i>
                  <p class="text-muted mt-3">No hay reportes <?= $userRole === 'administrador' ? 'en el sistema' : 'registrados' ?></p>
                  <?php if ($userRole !== 'administrador'): ?>
                  <a href="formulario.php" class="btn btn-primary mt-2">Crear primer reporte</a>
                  <?php endif; ?>
                </td>
              </tr>
            <?php endif; ?>
  ...
  <script>
    ...
    const colors = ['#4e73df', '#36b9cc', '#f6c23e', '#e74a3b', '#1cc88a', '#858796', '#5a5c69'];
